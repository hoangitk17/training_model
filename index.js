...
const testRealModel = document.getElementById("real-image");
...
function drawBoundingBoxes2(canvas, predictBoundingBox) {
  const ctx = canvas.getContext("2d");
  ctx.beginPath();
  ctx.strokeStyle = TRUE_BOUNDING_BOX_STYLE;
  ctx.lineWidth = TRUE_BOUNDING_BOX_LINE_WIDTH;
 ...
 *
 * @param {window.tf.Model} model Model to be used for inference.
 */
async function runAndVisualizeInference(model) {
  ...
  console.log(images, targets);
  const t0 = window.tf.util.now();
  // Runs inference with the model.
  const modelOut = await model.predict(images).data();
  inferenceTimeMs.textContent = `${(window.tf.util.now() - t0).toFixed(1)}`;
  ...
  window.tf.dispose([images, targets]);
}
async function runAndVisualizeInference2(model) {
  // Synthesize an input image and show it in the canvas.
  const ctx = canvas.getContext("2d");
  ...
  image.onload = async () => {
    ...
    const imageTensor = window.tf.browser.fromPixels(canvas);
    console.log(imageTensor);
    const t0 = window.tf.util.now();
    // Runs inference with the model.
    const modelOut = await model.predict(window.tf.stack([imageTensor])).data();
    inferenceTimeMs.textContent = `${(window.tf.util.now() - t0).toFixed(1)}`;

    // Visualize the true and predicted bounding boxes.

    drawBoundingBoxes2(canvas, modelOut.slice(1));
...
async function init() {
  ...
  try {
    ...
    model = await window.tf.loadLayersModel(LOCAL_MODEL_PATH);
    model.summary();
    ...
    console.log(err);
    status.textContent =
      "Failed to load locally-saved model. " +
  ...
  loadHostedModel.addEventListener("click", async () => {
    try {
      ...
      model = await window.tf.loadLayersModel(HOSTED_MODEL_PATH);
      model.summary();
      loadHostedModel.disabled = true;
      testModel.disabled = false;
      status.textContent = `Loaded hosted model successfully. Now click "Test Model".`;
      runAndVisualizeInference(model);
    } catch (err) {
      status.textContent = `Failed to load hosted model from ${HOSTED_MODEL_PATH}`;
    }
  });

  testModel.addEventListener("click", () => runAndVisualizeInference(model));
  testRealModel.addEventListener("click", () =>
    runAndVisualizeInference2(model)
  );
}
