...
class ObjectDetectionImageSynthesizer {
  ...
  async generateExample(numCircles, numLines, triangleProbability = 0.5) {
    ...
    return window.tf.tidy(() => {
      const imageTensor = window.tf.browser.fromPixels(this.canvas);
      const shapeClassIndicator = isRectangle ? 1 : 0;
      const targetTensor =
        window.tf.tensor1d([shapeClassIndicator].concat(boundingBox));
      console.log("test", imageTensor)
  ...
  async generateExampleBatch(
    ...
    const images = window.tf.stack(imageTensors);
    const targets = window.tf.stack(targetTensors);
    window.tf.dispose([imageTensors, targetTensors]);
